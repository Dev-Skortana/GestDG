...
namespace Recherche_donnees_GESTDG.Formats.Classes
{
    class Create_with_format_time:ICreate_with_format
    {
        public string Create_condition(string champ, Object valeur, Enumerations_recherches.methodes_recherches methode_recherche)
        {
            ...
            resultat += methode_recherche == Enumerations_recherches.methodes_recherches.Egale_a ? $"='{donnees_time.ToString("hh:mm")}'" : (methode_recherche == Enumerations_recherches.methodes_recherches.Superieure ? $">='{donnees_time.ToString("hh:mm")}'" : $"<='{donnees_time.ToString("hh:mm")}'");
            return resultat;
        }
